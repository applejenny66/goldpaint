...
def add_obj(self):
     ...
     box_name = "areaA_box"
     scene.add_box(box_name, box_pose, size=(0.91, 0.47, 0.87))
...
def go_to_pose_goal(x, y, z):
    ...
    pose_goal.position.x = x #0.058
    pose_goal.position.y = y #0.522
    pose_goal.position.z = z #1.127
    pose_goal.orientation = Quaternion(*quaternion_from_euler(1.75, 0.00, 3.14, 'szyx'))
...
if __name__ == '__main__':
  ...
  rospy.init_node('attacking_areaA_pose')
  
  pose_srv = rospy.Service('attacking_areaA_pose', PoseSrv, planner)
