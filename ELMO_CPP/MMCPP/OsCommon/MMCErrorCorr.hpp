...
public:
	 ...
	 */
	ELMO_INT32 LoadErrorCorrTable(MMC_LOADERRORTABLE_IN& stInParams) throw (CMMCException);
	 ...
	 * \return	0 on success, otherwise error or exception thrown.
	 */
	void LoadErrorCorrTable(ELMO_INT8 pPathToETFile[NC_MAX_ET_FILE_PATH_LENGTH], NC_ERROR_TABLE_NUMBER eETNumber, ELMO_DOUBLE dMaxCorrectionDelta = 0) throw (CMMCException);

	/**
	 * \fn	LoadErrorCorrTable(NC_ERROR_TABLE_NUMBER eETNumber, double dMaxCorrectionDelta = 0)
	 * \brief	this method loads an error correction table.
	 * \param	eETNumber table  index in journal.
	 * \param	dMaxCorrectionDelta maximum allowed aberration(error delta). default is 0 as for no aberration allowed.
	 * \return	0 on success, otherwise error or exception thrown.
	 */
	void LoadErrorCorrTable(NC_ERROR_TABLE_NUMBER eETNumber, ELMO_DOUBLE dMaxCorrectionDelta = 0) throw (CMMCException);
