...
public:
	...
	* \return - 0 if completed successfully, otherwise error or throws CMMCException
	*/
	ELMO_INT32 TrackConveyorBelt(MMC_TRACKCONVEYORBELT_IN* pInParam) throw (CMMCException);
	/**
	...
	* \return - 0 if completed successfully, otherwise error or throws CMMCException
	*/
	ELMO_INT32 TrackConveyorBelt(ELMO_UINT16 usRefAxis,	
						ELMO_DOUBLE (&dbConveyorBeltOrigin)[6], 
						ELMO_DOUBLE (&dbInitialObjectPosition)[6],
						ELMO_DOUBLE dInitialRefAxisPosition, 
						ELMO_DOUBLE dRefAxisScaling = 1.0,
	...
	* \return - 0 if completed successfully, otherwise error or throws CMMCException
	*/
	ELMO_INT32 TrackRotaryTable(MMC_TRACKROTARYTABLE_IN* pInParam) throw (CMMCException);
	/**
	...
	* \return - 0 if completed successfully, otherwise error or throws CMMCException
	*/
	ELMO_INT32 TrackRotaryTable(ELMO_UINT16 usRefAxis,	
						ELMO_DOUBLE (&dbRotaryTableOrigin)[6], 
						ELMO_DOUBLE (&dbInitialObjectPosition)[6],
						ELMO_DOUBLE dInitialRefAxisPosition, 
						ELMO_DOUBLE dRefAxisScaling = 1.0,
						PCS_REF_AXIS_SRC_ENUM eSourceType = NC_PCS_TARGET_POS,
						...
						MC_EXECUTION_MODE eExecutionMode = eMMC_EXECUTION_MODE_QUEUED) throw (CMMCException) ;
	/**
	* \fn	int TrackRotaryTableEx(unsigned short usMaster,	...)
	* \brief	this function block offers an abstraction layer for a rotary table,
	*			assisting the user with tracking objects moving on a cyrcle space.
	*			in short, a dynamic MCS to PCS transition depends on rotary table axis position.
	...
	* \return - 0 if completed successfully, otherwise error or throws CMMCException
	*/
	ELMO_INT32 TrackRotaryTableEx(unsigned short usMaster,
	...
	/**
	* \fn	int TrackRotaryTableEx(MMC_SYNCIN_IN& params)
	* \brief	this function block offers an abstraction layer for a rotary table,
	*			assisting the user with tracking objects moving on a cyrcle space.
	*			in short, a dynamic MCS to PCS transition depends on rotary table axis position.
	*			this command operates a real motion profiler, opposed to 'TrackRotaryTable', which is a command of type 'admin'.
	...
	* \return - 0 if completed successfully, otherwise error or throws CMMCException
	*/
	ELMO_INT32 TrackRotaryTableEx(MMC_TRACKSYNCIN_IN& params) throw (CMMCException);
		
	/**
	* \fn	int TrackConveyorBeltEx(unsigned short usMaster,	...)
	...
	* \return - 0 if completed successfully, otherwise error or throws CMMCException
	*/
	ELMO_INT32 TrackConveyorBeltEx(ELMO_UINT16 usMaster,
		ELMO_DOUBLE(&dbMasterOrigin)[6],
		ELMO_DOUBLE(&dbPCSOrigin)[6],
		ELMO_DOUBLE(&dbInitialObjectPosition)[6],
		ELMO_DOUBLE(&dbRampTrajectoryParams)[12],
		...
		PCS_REF_AXIS_SRC_ENUM eSourceType = NC_PCS_TARGET_POS) throw (CMMCException);
	/**
	* \fn	int TrackConveyorBeltEx(MMC_SYNCIN_IN& params)
	* \brief	this function block offers an abstraction layer for a conveyor belt,
	*			assisting the user with tracking objects moving on a cyrcle space.
	*			in short, a dynamic MCS to PCS transition depends on conveyor axis position.
	*			this command operates a real motion profiler, opposed to 'TrackConveyorBelt', which is a command of type 'admin'.
	...
	* \return - 0 if completed successfully, otherwise error or throws CMMCException
	*/
	ELMO_INT32 TrackConveyorBeltEx(MMC_TRACKSYNCIN_IN& params) throw (CMMCException);
...
//#ifndef WIN32
	void SetCartesianKinematics (MC_KIN_REF_CARTESIAN stCart)   throw (CMMCException) ;
	void SetDeltaRobotKinematics(MC_KIN_REF_DELTA 	  stDelta) 	throw (CMMCException) ;
	void SetKinematic(MC_KIN_REF stInput,NC_KIN_TYPE  eKinType) throw (CMMCException) ;
...
//#else
//    #error "***MMCGroupAxis.hpp: Supporting Env. Visual or Eclipse (found Env. from defined platform...). "
//#endif
	...
	*	\return 0 if completed successfully, otherwise error or throws CMMCException.
	*/
	int ReadKinTransform(MMC_READKINTRANSFORMEX_OUT& stOutParam) throw (CMMCException);
