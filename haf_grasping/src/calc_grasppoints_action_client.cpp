...
public:
	...
	ros::Subscriber camera_pc_sub;						//subscriber for pointcloud from camera
	ros::Subscriber pc_sub_bbox;						//subscriber for pointcloud with bbox specified
	...
	ros::ServiceServer srv_set_bbox_and_pc_sub;			//subscriber for pointcloud with bbox specified
	...
	float grasp_depth;					// define the distance between the camera frame and the point cloud
	float real_width, real_height;		// define the size of the camera view in the real frame
	...
	void get_camera_pc_cb(sensor_msgs::PointCloud2 pc_in);
	...
	CCalcGrasppointsClient(ros::NodeHandle nh_)
	{
		...
		//ROS_INFO("base_frame: %s", base_frame_name.c_str());
		nh_.param("/haf_grasping/base_frame", this->base_frame_default, base_frame_name);
		// set gripper opening with factor => 1/gripper opening width is tested
		nh_.param("/haf_grasping/gripper_width", this->gripper_opening_width, 1);
		// set parameters for RGB  frame to point cloud frame
		nh_.getParam("/haf_grasping/grasp_depth", this->grasp_depth);
		nh_.getParam("/haf_grasping/real_width", this->real_width);
		nh_.getParam("/haf_grasping/real_height", this->real_height);
		ROS_INFO("param initialize %f %f %f", this->grasp_depth, this->real_width, this->real_height);
		...
		this->camera_pc_sub = nh_.subscribe("/c1/camera/depth/points",1, &CCalcGrasppointsClient::get_camera_pc_cb, this);
		...
		this->srv_set_bbox_and_pc_sub = nh_.advertiseService("/haf_grasping/GraspPoseEst", &CCalcGrasppointsClient::set_bbox_and_pc_sub, this);
	}
};
...
void CCalcGrasppointsClient::get_camera_pc_cb(sensor_msgs::PointCloud2 pc_in)
{
...
// set grasp search area as bbox and start get_grasp_cb (that triggers grasp calculation)
bool CCalcGrasppointsClient::set_bbox_and_pc_sub(pmc_msgs::GraspPoseEst_direct::Request &req,
		pmc_msgs::GraspPoseEst_direct::Response &res)
{
	pcl::PointXYZ rgb_center(((1 - req.bbox_corner1.x)+(1 - req.bbox_corner2.x))/2,
							 ((1 - req.bbox_corner1.y)+(1 - req.bbox_corner2.y))/2,
							 (req.bbox_corner1.z+req.bbox_corner2.z)/2);
	float rgb_width = fabs((1 - req.bbox_corner1.x)-(1 - req.bbox_corner2.x));
	float rgb_height = fabs((1 - req.bbox_corner1.y)-(1 - req.bbox_corner2.y));
	float pc_width = rgb_width * this->real_width * 100;
	float pc_height = rgb_height * this->real_height * 100;
	ROS_INFO("%f, %f, %f, %f", rgb_width, rgb_height, pc_width, pc_height);

	//set grasp search center
	this->graspsearchcenter.x = this->grasp_depth;
	this->graspsearchcenter.y = -this->real_width/2 + this->real_width * rgb_center.x;
	this->graspsearchcenter.z = -this->real_height/2 + this->real_height * rgb_center.y;
	//this->graspsearchcenter = rgb_center;
	ROS_INFO("Set grasp search center to: x=%f, y=%f, z=%f", this->graspsearchcenter.x, this->graspsearchcenter.y, this->graspsearchcenter.z);

	//set grasp search rectangle size
	if (pc_width >= 0 and pc_width <= this->max_grasp_search_size_x)
	{
		this->grasp_search_size_x = pc_width;
		ROS_INFO("Set grasp rectangle size to: x=%ld", (long int)pc_width);
	}
	else
	{
		ROS_INFO("Could not set grasp rectangle size for x. Allowed values: [0, %ld ]. Received value was: x=%ld", (long int)this->max_grasp_search_size_x,(long int)pc_width);
		this->grasp_search_size_x = this->max_grasp_search_size_x;
		//return false;
	}
	if (pc_height >= 0 and pc_height <= this->max_grasp_search_size_y)
	{
		this->grasp_search_size_y = pc_height;
		ROS_INFO("Set grasp rectangle size to: y=%ld", (long int)pc_height);
	}
	else
	{
		ROS_INFO("Could not set grasp rectangle size for y. Allowed values: [0, %ld ]. Received value was: y=%ld", (long int)this->max_grasp_search_size_y,(long int)pc_height);
		//return false;
		this->grasp_search_size_y = this->max_grasp_search_size_y;
	}

	const sensor_msgs::PointCloud2ConstPtr input_pc_const_ptr = boost::make_shared<sensor_msgs::PointCloud2>(this->camera_pc);
	// CCalcGrasppointsClient::get_grasp_cb(input_pc_const_ptr);
	// create the action client
	// true causes the client to spin its own thread
	actionlib::SimpleActionClient<haf_grasping::CalcGraspPointsServerAction> ac("calc_grasppoints_svm_action_server", true);
	ROS_INFO("Waiting for action server to start.");
	...
	goal.graspinput.grasp_area_center = this->graspsearchcenter;
#if 0
  std::cout << this->graspsearchcenter.x << ", " <<
               this->graspsearchcenter.y << ", " <<
               this->graspsearchcenter.z << std::endl;
#endif
	goal.graspinput.approach_vector = this->approach_vector;
#if 0
  std::cout << this->approach_vector.x << ", " <<
               this->approach_vector.y << ", " <<
               this->approach_vector.z << std::endl;
#endif
	goal.graspinput.grasp_area_length_x = this->grasp_search_size_x+14;
	goal.graspinput.grasp_area_length_y = this->grasp_search_size_y+14;
	goal.graspinput.max_calculation_time = this->grasp_calculation_time_max;
	...
	if (finished_before_timeout)
	{
		actionlib::SimpleClientGoalState state = ac.getState();
		boost::shared_ptr<const haf_grasping::CalcGraspPointsServerResult_<std::allocator<void> > > result = ac.getResult();
		res.grasp_pose.position = (*(result)).graspOutput.graspPoint1;
